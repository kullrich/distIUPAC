...
faidx2seq<-function(fasta, start=NULL, end=NULL, format="dna", samtools="samtools"){
  options(scipen=22)
  samples<-read.table(paste0(fasta,".fai"),sep="\t",header=FALSE,stringsAsFactors=FALSE)[,1]
  len<-unique(read.table(paste0(fasta,".fai"),sep="\t",header=FALSE,stringsAsFactors=FALSE)[,2])
