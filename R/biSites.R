#' @title biSites
#' @name biSites
#' @description This function returns bi-allelic site positions given a dna object
...
biSites<-function(dna, x.pos=NULL, min.ind=0, wlen=25000, start.by=NULL, end.by=NULL, threads=1, pB=TRUE){
  ...
  OUT<-foreach(j=1:dim(tmp.sw)[2], .combine=rbind) %dopar% {
    ...
    biPOSall<-START-1+which(apply(tmp.seq.cM,1,function(x) length(unique(unlist(unique(IUPAC_CODE_MAP_LIST[names(x[x==1])])))))==2)
